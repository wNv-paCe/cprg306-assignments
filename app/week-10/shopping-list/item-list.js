...
export function ItemList({ items, onItemSelect, onUpdatedItem, onDeleteItem }) {
              ...
              <ul>
                {sortedItems[category].map((item) => (
                  <Item
                    ...
                    onUpdate={(updateData) =>
                      onUpdatedItem(item.id, updateData)
                    }
        ...
        <ul>
          {sortedItems.map((item) => (
            <Item
              ...
              category={item.category}
              onSelect={() => onItemSelect(item)}
              onUpdate={(updateData) => onUpdatedItem(item.id, updateData)}
              onDelete={() => onDeleteItem(item.id)}
            />
          ))}
        </ul>
